...
function add_pages($dir,$parent_id,$page_path_parent){
	//note: recursive, do not change arg values here!!!
	...
	foreach ($files as $file) {
		...
		//$file_exists=0;
		if( $wp_post ){$file_exists = 1;} //skip post_page_if_required if this file page already exists

		if(is_file($dir.'/'.$file)){
			$post_status = 'draft';
			if(! $parent_id){$post_status = 'publish';}//only publish if root pages and files
			$load_data = 1;
			if(! $wp_post)  $parent_id_temp =  post_page_if_required( $parent_id , $page_path_parent , $file , $dir , $post_status , $load_data);
		}
		if(is_dir($dir.'/'.$file)){
			//create empty page
			$post_status = 'draft';
